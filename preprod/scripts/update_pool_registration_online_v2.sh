...
# Config
# =========================
ROOT_PATH="/home/cardano"
TOPO_FILE="${ROOT_PATH}/pool_topology"

NODE_DIR="${ROOT_PATH}/node.bp"
SOCKET_PATH="${NODE_DIR}/socket/node.socket"
...
# Pool metadata
META_FILENAME_DEFAULT="adact_preprod.json"
...
- Uses DNS relays via: --pool-relay-dns <dns> --pool-relay-port 3001
...
# Parse pool_topology:
# - bp line:   <lan_ip> bp
# - relay line:<lan_ip> <relayName> <publicDNS>
read_topology() {
...
# =========================
# Locate & source utils.sh early (needed for get_network_magic)
# =========================
...
echo "  META_HASH:    $META_HASH"
echo "  RELAYS (DNS): ${RELAY_DNS[*]} port 3001"
echo
...
RELAY_PARAMS=()
for dns in "${RELAY_DNS[@]}"; do
  RELAY_PARAMS+=( --single-host-pool-relay "$dns" --pool-relay-port 3001 )
done
