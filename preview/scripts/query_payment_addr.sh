...
if [[ $ADV_FLAG == "N" ]]; then
    ...
    cardano-cli query utxo --address $PAYMENT_ADDR --socket-path /home/cardano/data/preview/node.bp/socket/node.socket --testnet-magic $MAGIC
else
    ...
    cardano-cli query utxo --address $PAYMENT_ADDR --socket-path /home/cardano/data/preview/node.bp/socket/node.socket --testnet-magic $MAGIC > /tmp/query_payment_addr.out
