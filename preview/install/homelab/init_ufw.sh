...
if [[ "$NODE_TYPE" == "relay" ]]; then
  ...
  log "Configuring HYBRID rules (BP + Relay on same host)..."

  # Relay: open relay port to WAN (relay must be publicly reachable)
  run "ufw allow $RELAY_PORT/tcp"

  # BP: restrict BP port. Local relay uses loopback (already allowed by 'allow in on lo').
  # Allow any additional remote relay IPs to reach BP port.
  for rip in "${RELAY_IPS[@]}"; do
    [[ "$rip" == "$LOCAL_IP" ]] && continue
    run "ufw allow from $rip to any port $BP_PORT proto tcp"
  done

  # SSH: use BP-level security (LAN-only default)
  if [[ -z "$ALLOW_SSH_FROM" ]]; then
    log "SSH: Hybrid default to LAN-only (192.168.10.0/24). Override via ALLOW_SSH_FROM if needed."
    run "ufw allow from 192.168.10.0/24 to any port $SSH_PORT proto tcp"
  else
    apply_ssh_rules "hybrid"
  fi
