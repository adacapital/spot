...
git clone https://github.com/midnightntwrk/midnight-node-docker.git midnight-node-docker2
# git clone git@github.com:midnightntwrk/midnight-node-docker.git midnight-node-docker2
cd midnight-node-docker2
...
# Testnet=02
# Make sure that keys are in the right place in the midnight container

# # start interactive bash shell of midnight-node container
# docker exec -it <conainerid> bash

# # Make sure all keys have 644 permissions -rw-r--r--
...
# # restart the midnight node
# docker compose down
# docker compose up -d

# # verify block production
# docker logs <containerId> |& grep Prepared 