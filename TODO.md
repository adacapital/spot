...
- **Pick up `peer-snapshot.json` for mainnet during the eventual OCI→homelab migration.**
  *Discovered: 2026-05-20.* Mainnet's existing topology.json doesn't reference `peerSnapshotFile` — predates the feature (mainnet OCI install is older than preprod/preview homelab installs which got it Feb 2026). Rather than modify in place, plan to pick it up as part of the OCI→homelab migration: fresh VMs, fresh IOG-published config and topology straight from `book.play.dev.cardano.org/environments/mainnet/`, peer-snapshot.json baked in by default. Same approach as preprod's Feb 2026 homelab migration. The resilience win: future extended downtime crossing a hard fork becomes Mithril-bootstrap-recoverable (the apollo Van Rossem story doesn't repeat).
