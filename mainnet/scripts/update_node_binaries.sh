...
if [[ $NODE_TYPE == "bp" ]]; then
    ...
    # Mainnet uses per-host SSH keys (OCI convention): ~/.ssh/adact-mainnet-<relayname>
    # SSH options harden the call: explicit identity, accept new host keys, fail fast.
    ...
    remote() {
        ...
        ssh $SSH_OPTS -i "$key" "cardano@${ip}" "$cmd"
    }

    remote_scp_to() {
        local ip="$1" key="$2" src="$3" dst="$4"
        scp $SCP_OPTS -i "$key" "$src" "cardano@${ip}:${dst}"
    }
    ...
    for (( i=0; i<${RELAYS_COUNT}; i++ )); do
        ip="${RELAY_IPS[$i]}"
        name="${RELAY_NAMES[$i]}"
        ssh_key="$HOME/.ssh/adact-mainnet-${name}"

        echo
        echo "Relay $name ($ip): using key $ssh_key"

        if [[ ! -f "$ssh_key" ]]; then
            echo "ERROR: SSH key not found at $ssh_key — skipping $name."
            continue
        fi

        echo "Relay $name ($ip): checking SSH connectivity..."
        if ! remote "$ip" "$ssh_key" "echo ok >/dev/null"; then
            echo "Relay $name ($ip): unreachable via SSH (with $ssh_key), skipping."
            continue
        fi

        echo "Relay $name ($ip): stopping relay service..."
        remote "$ip" "$ssh_key" "sudo -n systemctl stop run.relay"

        echo "Relay $name ($ip): copying cardano binaries..."
        remote_scp_to "$ip" "$ssh_key" "$HOME/.local/bin/cardano-node" "/home/cardano/.local/bin/cardano-node"
        remote_scp_to "$ip" "$ssh_key" "$HOME/.local/bin/cardano-cli"  "/home/cardano/.local/bin/cardano-cli"
        remote "$ip" "$ssh_key" "chmod 755 ~/.local/bin/cardano-node ~/.local/bin/cardano-cli && ~/.local/bin/cardano-node --version && ~/.local/bin/cardano-cli --version"

        echo "Relay $name ($ip): starting relay service..."
        remote "$ip" "$ssh_key" "sudo -n systemctl start run.relay"
